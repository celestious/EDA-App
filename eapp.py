...
def main():
	...
	menu = ["Home","Pandas Profile","Sweetviz","Pandas", "Seaborn","Dtale", "PandasGUI", "Plotly", "About"]
	...
	if choice == "Pandas Profile":
		...
		if data_file is not None:
			...
			profile = ProfileReport(data_file)
			#profile = ProfileReport(df, title='Your Pandas profile report', html={'style': {"full_width": True}}, sort="None")
	...
	elif choice == "PandasGUI":
		st.subheader("Automated EDA with PandasGUI")
		data_file = st.file_uploader("Upload CSV", type=['csv'])
		if data_file is not None:
			df = pd.read_csv(data_file)
			st.dataframe(df.head())
		...
		if data_file is not None:
			...
			st.dataframe(df.describe())
			#if st.button("Generate Pandas Report")


	elif choice == "Seaborn":
		st.subheader("Automated EDA with Seaborn")
		data_file = st.file_uploader("Upload CSV", type=['csv'])
		if data_file is not None:
			df = pd.read_csv(data_file) 
			sns.pairplot(df)
